...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Tangani data POST disini, misalnya simpan ke database atau proses data
    $data1 = $_POST['data1'];
    $data2 = $_POST['data2'];
    // Lakukan sesuatu dengan data, misalnya menyimpan ke database
    // Contoh: simpanDataKeDatabase($data1, $data2);

    // Set pesan status
    $status_message = "Data berhasil dikirim!";
}
?>
...
<div class="container mt-5">
    <div class="row">
        <div class="col-12 text-end">
            <a class="btn btn-outline-primary btn-sm mb-0" href="../pages/antrian.php">Buat Jadwal</a>
        </div>
    </div>
    <div class="row mt-4">
        <div class="col-12">
            ...
            <div class="form-group">
                        <?php
                        // Koneksi ke database
                        $conn = new mysqli("localhost", "root", "", "healt_plus_data");
                        if ($conn->connect_error) {
                            die("Connection failed: " . $conn->connect_error);
                        }

                        // Ambil no_antrian tertinggi
                        $sql = "SELECT MAX(no_antrian) AS max_antrian FROM daftar_poli";
                        $result = $conn->query($sql);
                        $next_antrian = 1; // Default value if no rows found
                        if ($result->num_rows > 0) {
                            $row = $result->fetch_assoc();
                            $next_antrian = $row['max_antrian'] + 1;
                        }
